/**
 * Unit tests for GET /api/healthz-smoke-688707801
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 688707801.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "688707801" }
...
describe('GET /api/healthz-smoke-688707801', () => {
  beforeEach(() => {
  ...
  // AC-03: Response body matches spec: { ok: true, variant: "688707801" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('688707801');
  });
  ...
  // AC-06: variant field is string "688707801" (not number)
  it('RH-06: variant field is string "688707801" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('688707801');
    expect(json.variant).toBe('688707801');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '688707801' };
    bodies.forEach((body) => {
