/**
 * Unit tests for GET /healthz-smoke-bugfix-629775393
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 629775393.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "629775393" }
...
describe('GET /healthz-smoke-bugfix-629775393', () => {
  beforeEach(() => {
  ...
  // AC-03: Response body matches spec: { ok: true, variant: "629775393" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('629775393');
  });
  ...
  // AC-06: variant field is string "629775393" (not number)
  it('RH-06: variant field is string "629775393" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('629775393');
    expect(json.variant).toBe('629775393');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '629775393' };
    bodies.forEach((body) => {
