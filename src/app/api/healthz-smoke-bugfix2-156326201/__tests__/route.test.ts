/**
 * Unit tests for GET /api/healthz-smoke-bugfix2-156326201
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 156326201.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "156326201" }
...
describe('GET /api/healthz-smoke-bugfix2-156326201', () => {
  beforeEach(() => {
  ...
  // AC-03: Response body matches spec: { ok: true, variant: "156326201" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('156326201');
  });
  ...
  // AC-06: variant field is string "156326201" (not number)
  it('RH-06: variant field is string "156326201" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('156326201');
    expect(json.variant).toBe('156326201');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '156326201' };
    bodies.forEach((body) => {
