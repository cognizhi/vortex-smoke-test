/**
 * Unit tests for GET /api/healthz-smoke-bugfix2-446144862
 *
 * Smoke test endpoint for load balancers and monitoring systems.
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "446144862" }
...
describe('GET /api/healthz-smoke-bugfix2-446144862', () => {
  beforeEach(() => {
  ...
  it('TC-003: variant field is string "446144862"', async () => {
    const res = await GET();
    const json = (await res.json()) as { ok: boolean; variant: string };
    expect(json.variant).toBe('446144862');
  ...
  it('additional: response has exact shape { ok: true, variant: "446144862" }', async () => {
    const res = await GET();
    const json = await res.json();
    expect(json).toEqual({ ok: true, variant: '446144862' });
  });
