/**
 * Unit tests for GET /api/healthz-smoke-bugfix-906735349
 *
 * Smoke test endpoint for load balancers and monitoring systems.
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "906735349" }
...
describe('GET /api/healthz-smoke-bugfix-906735349', () => {
  beforeEach(() => {
  ...
  it('TC-003: variant field is string "906735349"', async () => {
    const res = await GET();
    const json = (await res.json()) as { ok: boolean; variant: string };
    expect(json.variant).toBe('906735349');
  ...
  it('additional: response has exact shape { ok: true, variant: "906735349" }', async () => {
    const res = await GET();
    const json = await res.json();
    expect(json).toEqual({ ok: true, variant: '906735349' });
  });
