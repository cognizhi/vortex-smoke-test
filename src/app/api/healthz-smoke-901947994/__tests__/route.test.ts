/**
 * Unit tests for GET /api/healthz-smoke-901947994
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 901947994.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "901947994" }
...
describe('GET /api/healthz-smoke-901947994', () => {
  beforeEach(() => {
  ...
  // AC-03: Response body matches spec: { ok: true, variant: "901947994" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('901947994');
  });
  ...
  // AC-06: variant field is string "901947994" (not number)
  it('RH-06: variant field is string "901947994" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('901947994');
    expect(json.variant).toBe('901947994');
  });
