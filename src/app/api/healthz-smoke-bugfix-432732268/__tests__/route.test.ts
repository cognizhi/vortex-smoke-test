/**
 * Unit tests for GET /api/healthz-smoke-bugfix-432732268
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 432732268.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "432732268" }
...
describe('GET /api/healthz-smoke-bugfix-432732268', () => {
  beforeEach(() => {
  ...
  // AC-02: Response body matches spec: { ok: true, variant: "432732268" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('432732268');
  });
  ...
  // AC-06: variant field is string "432732268" (not number)
  it('RH-06: variant field is string "432732268" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('432732268');
    expect(json.variant).toBe('432732268');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '432732268' };
    bodies.forEach((body) => {
