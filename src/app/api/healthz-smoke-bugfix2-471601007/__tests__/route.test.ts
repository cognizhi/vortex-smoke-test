/**
 * Regression test for GET /healthz-smoke-bugfix2-471601007
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 471601007.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "471601007" }
...
describe('GET /healthz-smoke-bugfix2-471601007', () => {
  beforeEach(() => {
  ...
  // AC-03: Response body matches spec: { ok: true, variant: "471601007" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('471601007');
  });
  ...
  // AC-06: variant field is string "471601007" (not number)
  it('RH-06: variant field is string "471601007" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('471601007');
    expect(json.variant).toBe('471601007');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '471601007' };
    bodies.forEach((body) => {
