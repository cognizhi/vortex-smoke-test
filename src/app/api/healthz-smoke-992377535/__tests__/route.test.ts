/**
 * Unit tests for GET /api/healthz-smoke-992377535
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 992377535.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { data: { ok: true, variant: "992377535" }, error: null }
...
describe('GET /api/healthz-smoke-992377535', () => {
  beforeEach(() => {
  ...
  // AC-01: Response body matches spec: { data: { ok: true, variant: "992377535" }, error: null }
  it('RH-02: returns correct JSON structure with data and error', async () => {
    ...
    expect(json.data.variant).toBe('992377535');
  ...
  // AC-02: data.variant field is string "992377535" (not number)
  it('RH-05: data.variant field is string "992377535" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { data: { variant: unknown } };
    expect(typeof json.data.variant).toBe('string');
    expect(json.data.variant).toStrictEqual('992377535');
    expect(json.data.variant).toBe('992377535');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { data: { ok: true, variant: '992377535' }, error: null };
    bodies.forEach((body) => {
