/**
 * Unit tests for GET /api/healthz-smoke-bugfix-254027906
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 254027906.
 *
 * REGRESSION TEST for VRTX-0289: Verifies that /healthz-smoke-bugfix-254027906
 * returns 200 with correct JSON structure, not 404.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "254027906" }
...
describe('GET /api/healthz-smoke-bugfix-254027906', () => {
  beforeEach(() => {
  ...
  // AC-02: Response body matches spec: { ok: true, variant: "254027906" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('254027906');
  });
  ...
  // AC-06: variant field is string "254027906" (not number)
  it('RH-06: variant field is string "254027906" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('254027906');
    expect(json.variant).toBe('254027906');
  });
  ...
  it('RH-12: multiple calls return consistent response', async () => {
    ...
    expect(json1.variant).toBe('254027906');
    expect(json2.variant).toBe('254027906');
  });
