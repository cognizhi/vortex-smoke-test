/**
 * Unit tests for GET /api/healthz-smoke-96685
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 96685.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { data: { ok: true, variant: "96685" }, error: null }
...
describe('GET /api/healthz-smoke-96685', () => {
  beforeEach(() => {
  ...
  // AC-01: Response body matches spec: { data: { ok: true, variant: "96685" }, error: null }
  it('RH-02: returns correct JSON structure with data and error', async () => {
    ...
    expect(json.data.variant).toBe('96685');
  ...
  // AC-02: data.variant field is string "96685" (not number)
  it('RH-05: data.variant field is string "96685" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { data: { variant: unknown } };
    expect(typeof json.data.variant).toBe('string');
    expect(json.data.variant).toStrictEqual('96685');
    expect(json.data.variant).toBe('96685');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { data: { ok: true, variant: '96685' }, error: null };
    bodies.forEach((body) => {
