/**
 * Unit tests for GET /api/healthz-smoke-bugfix2-1007381648
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 1007381648.
 *
 * REGRESSION TEST for VRTX-0211: Verifies that /healthz-smoke-bugfix2-1007381648
 * endpoint returns 200 with correct variant identification.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "1007381648" }
...
describe('GET /api/healthz-smoke-bugfix2-1007381648', () => {
  beforeEach(() => {
  ...
  // AC-01: Response body matches spec: { ok: true, variant: "1007381648" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('1007381648');
  });

  // AC-01: No extra fields in root object
  ...
  it('RH-04: ok field is boolean true (not just truthy)', async () => {
    const res = await GET();
  ...
  // AC-02: variant field is string "1007381648" (not number)
  it('RH-05: variant field is string "1007381648" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('1007381648');
    expect(json.variant).toBe('1007381648');
  });
  ...
  it('RH-11: endpoint requires no authentication', async () => {
  ...
  it('RH-12: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '1007381648' };
    bodies.forEach((body) => {
