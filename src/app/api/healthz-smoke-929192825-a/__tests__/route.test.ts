...
describe('GET /api/healthz-smoke-929192825-a', () => {
  it('returns 200 with correct JSON', async () => {
    const request = new NextRequest('http://localhost:3000/api/healthz-smoke-929192825-a', {
      method: 'GET',
    });
  ...
  it('variant field contains the correct value', async () => {
    const request = new NextRequest('http://localhost:3000/api/healthz-smoke-929192825-a', {
      method: 'GET',
    });
    const response = await GET(request);
