...
describe('/api/healthz-smoke-1012136249-a', () => {
  let mockRequest: NextRequest

  beforeEach(() => {
    mockRequest = new NextRequest(
      new URL('http://localhost:3000/api/healthz-smoke-1012136249-a')
    )
  })
  ...
  it('response body contains variant: "1012136249"', async () => {
    const response = await GET(mockRequest)
    const body = await response.json()
    expect(body.variant).toBe('1012136249')
  })
  ...
  it('response structure matches exact spec { ok: true, variant: "1012136249" }', async () => {
    const response = await GET(mockRequest)
    const body = await response.json()
