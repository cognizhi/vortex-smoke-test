/**
 * Unit tests for GET /api/healthz-smoke-572185676
 *
 * Smoke test endpoint variant for load balancers and monitoring systems.
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "572185676" }
...
describe('GET /api/healthz-smoke-572185676', () => {
  ...
  // AC-02: Response body matches spec: { ok: true, variant: "572185676" }
  ...
  it('RH-05: multiple sequential calls return consistent responses', async () => {
    ...
    expect(json1).toEqual({ ok: true, variant: '572185676' });
    expect(json2).toEqual({ ok: true, variant: '572185676' });
    expect(json3).toEqual({ ok: true, variant: '572185676' });
  });
