/**
 * Unit tests for GET /api/healthz-smoke-bugfix2-407985318
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 407985318.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "407985318" }
...
describe('GET /api/healthz-smoke-bugfix2-407985318', () => {
  beforeEach(() => {
  ...
  // AC-02: Response body matches spec: { ok: true, variant: "407985318" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('407985318');
  });
  ...
  // AC-06: variant field is string "407985318" (not number)
  it('RH-06: variant field is string "407985318" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('407985318');
    expect(json.variant).toBe('407985318');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '407985318' };
    bodies.forEach((body) => {
