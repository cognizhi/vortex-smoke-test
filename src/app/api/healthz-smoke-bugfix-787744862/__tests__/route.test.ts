/**
 * Unit tests for GET /api/healthz-smoke-bugfix-787744862
 *
 * Smoke test endpoint for load balancers and monitoring systems.
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "787744862" }
...
describe('GET /api/healthz-smoke-bugfix-787744862', () => {
  beforeEach(() => {
  ...
  it('TC-003: variant field is string "787744862"', async () => {
    const res = await GET();
    const json = (await res.json()) as { ok: boolean; variant: string };
    expect(json.variant).toBe('787744862');
  ...
  it('additional: response has exact shape { ok: true, variant: "787744862" }', async () => {
    const res = await GET();
    const json = await res.json();
    expect(json).toEqual({ ok: true, variant: '787744862' });
  });
