/**
 * Unit tests for GET /api/healthz-smoke-bugfix2-691130485
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 691130485.
 *
 * REGRESSION TEST for VRTX-0367: Verifies that /healthz-smoke-bugfix2-691130485
 * endpoint returns 200 with correct variant identification.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "691130485" }
...
describe('GET /api/healthz-smoke-bugfix2-691130485', () => {
  beforeEach(() => {
  ...
  // AC-01: Response body matches spec: { ok: true, variant: "691130485" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('691130485');
  });
  ...
  // AC-02: variant field is string "691130485" (not number)
  it('RH-05: variant field is string "691130485" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('691130485');
    expect(json.variant).toBe('691130485');
  });
  ...
  it('RH-12: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '691130485' };
    bodies.forEach((body) => {
