 ...
 * This variant build is identified by the code 887203910.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "887203910" }
...
describe('GET /api/healthz-smoke-bugfix2-887203910', () => {
  ...
  // AC-03: Response body matches spec: { ok: true, variant: "887203910" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('887203910');
  });
  ...
  // AC-06: variant field is string "887203910" (not number)
  it('RH-06: variant field is string "887203910" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('887203910');
    expect(json.variant).toBe('887203910');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '887203910' };
    bodies.forEach((body) => {
