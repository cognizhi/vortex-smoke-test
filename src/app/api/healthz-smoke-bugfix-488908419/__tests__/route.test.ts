/**
 * Unit tests for GET /api/healthz-smoke-bugfix-488908419
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 488908419.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "488908419" }
...
describe('GET /api/healthz-smoke-bugfix-488908419', () => {
  beforeEach(() => {
  ...
  // AC-03: Response body matches spec: { ok: true, variant: "488908419" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('488908419');
  });
  ...
  // AC-06: variant field is string "488908419" (not number)
  it('RH-06: variant field is string "488908419" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('488908419');
    expect(json.variant).toBe('488908419');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '488908419' };
    bodies.forEach((body) => {
