/**
 * Unit tests for GET /api/healthz-smoke-bugfix-318187519
 *
 * Smoke test endpoint variant for load balancers and monitoring systems.
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "318187519" }
...
describe('GET /api/healthz-smoke-bugfix-318187519', () => {
  beforeEach(() => {
  ...
  // AC-02: Response body matches spec: { ok: true, variant: "318187519" }
  it('RH-02: returns correct JSON structure with ok: true and variant', async () => {
    ...
    expect(json.variant).toBe('318187519');
  });
  ...
  it('RH-05: multiple sequential calls return consistent responses', async () => {
    ...
    expect(json1).toEqual({ ok: true, variant: '318187519' });
    expect(json2).toEqual({ ok: true, variant: '318187519' });
    expect(json3).toEqual({ ok: true, variant: '318187519' });
  });
