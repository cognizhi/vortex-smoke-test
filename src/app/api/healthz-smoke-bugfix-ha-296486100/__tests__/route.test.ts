/**
 * Unit tests for GET /api/healthz-smoke-bugfix-ha-296486100
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 296486100.
 *
 * REGRESSION TEST for VRTX-0454: Verifies that /healthz-smoke-bugfix-ha-296486100
 * endpoint returns 200 with correct variant identification.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "296486100" }
...
describe('GET /api/healthz-smoke-bugfix-ha-296486100', () => {
  beforeEach(() => {
  ...
  // AC-01: Response body matches spec: { ok: true, variant: "296486100" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('296486100');
  });
  ...
  // AC-02: variant field is string "296486100" (not number)
  it('RH-05: variant field is string "296486100" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('296486100');
    expect(json.variant).toBe('296486100');
  });
  ...
  it('RH-12: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '296486100' };
    bodies.forEach((body) => {
