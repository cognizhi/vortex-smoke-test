/**
 * Unit tests for GET /api/healthz-smoke-bugfix-449792264
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 449792264.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "449792264" }
...
describe('GET /api/healthz-smoke-bugfix-449792264', () => {
  beforeEach(() => {
  ...
  // AC-02: Response body matches spec: { ok: true, variant: "449792264" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('449792264');
  });
  ...
  // AC-06: variant field is string "449792264" (not number)
  it('RH-06: variant field is string "449792264" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('449792264');
    expect(json.variant).toBe('449792264');
  });
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '449792264' };
    bodies.forEach((body) => {
