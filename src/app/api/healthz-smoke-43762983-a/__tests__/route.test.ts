/**
 * Unit tests for GET /api/healthz-smoke-43762983-a
 *
...
describe('GET /api/healthz-smoke-43762983-a', () => {
   ...
   * Verifies the specific variant is returned (not "43762983-a" or other variants)
   */
  it('RH-14: variant identifier is exactly "43762983"', async () => {
    const res = await GET();
    const json = (await res.json()) as { ok: boolean; variant: string };

    // Verify exact match — not "43762983-a", not different variant
    expect(json.variant).toBe('43762983');
    expect(json.variant).not.toBe('43762983-a');
