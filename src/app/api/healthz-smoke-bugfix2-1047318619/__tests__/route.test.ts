/**
 * Unit tests for GET /api/healthz-smoke-bugfix2-1047318619
 *
 * Variant health check endpoint for load balancers and monitoring systems.
 * This variant build is identified by the code 1047318619.
 *
 * Tests verify:
 *   - Returns 200 status code
 *   - Correct JSON response shape { ok: true, variant: "1047318619" }
...
describe('GET /api/healthz-smoke-bugfix2-1047318619', () => {
  beforeEach(() => {
  ...
  // AC-02: Response body matches spec: { ok: true, variant: "1047318619" }
  it('RH-02: returns correct JSON structure with ok and variant', async () => {
    ...
    expect(json.variant).toBe('1047318619');
  });
  ...
  // AC-06: variant field is string "1047318619" (not number)
  it('RH-06: variant field is string "1047318619" (not number)', async () => {
    const res = await GET();
    const json = (await res.json()) as { variant: unknown };
    expect(typeof json.variant).toBe('string');
    expect(json.variant).toStrictEqual('1047318619');
    expect(json.variant).toBe('1047318619');
  });
  ...
  // AC-08: Response is a NextResponse instance
  ...
  // AC-10: Response time typically < 10ms
  ...
  it('RH-13: multiple sequential calls return consistent responses', async () => {
    ...
    const expected = { ok: true, variant: '1047318619' };
    bodies.forEach((body) => {
      expect(body).toEqual(expected);
    });
  });

  // AC-14: No environment variables needed (self-contained)
