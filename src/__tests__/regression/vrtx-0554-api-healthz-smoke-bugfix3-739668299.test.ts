/**
 * Regression Test: VRTX-0554 - /api/healthz-smoke-bugfix3-739668299 returns 404 instead of 200
 *
 * Bug: API health check endpoint returns 404 instead of 200 with correct JSON response.
 * Root Cause: Missing route handler file at src/app/api/healthz-smoke-bugfix3-739668299/route.ts
 ...
 * 3. It returns the correct JSON response with variant identifier "739668299"
...
import { GET } from '../../app/api/healthz-smoke-bugfix3-739668299/route';

describe('REGRESSION: VRTX-0554 - API health check endpoint /api/healthz-smoke-bugfix3-739668299', () => {
  ...
  it('returns JSON response with ok=true and variant=739668299', async () => {
    const res = await GET();
    const json = await res.json();
    expect(json.ok).toBe(true);
    expect(json.variant).toBe('739668299');
  });

  it('returns exactly {"ok":true,"variant":"739668299"} with no extra fields', async () => {
    const res = await GET();
    const json = await res.json();
