/**
 * Regression Test: VRTX-0552 - /api/healthz-smoke-bugfix-863883409 returns 404 instead of 200
 *
 * Bug: API health check endpoint returns 404 instead of 200 with correct JSON response.
 * Root Cause: Missing route handler file at src/app/api/healthz-smoke-bugfix-863883409/route.ts
 ...
 * 3. It returns the correct JSON response with variant identifier "863883409"
...
import { GET } from '../../app/api/healthz-smoke-bugfix-863883409/route';

describe('REGRESSION: VRTX-0552 - API health check endpoint /api/healthz-smoke-bugfix-863883409', () => {
  ...
  it('returns JSON response with ok=true and variant=863883409', async () => {
    const res = await GET();
    const json = await res.json();
    expect(json.ok).toBe(true);
    expect(json.variant).toBe('863883409');
  });

  it('returns exactly {"ok":true,"variant":"863883409"} with no extra fields', async () => {
    const res = await GET();
    const json = await res.json();
