/**
 * Regression Test: VRTX-0560 - /api/healthz-smoke-bugfix3-163893398 returns 404 instead of 200
 *
 * Bug: API health check endpoint returns 404 instead of 200 with correct JSON response.
 * Root Cause: Missing route handler file at src/app/api/healthz-smoke-bugfix3-163893398/route.ts
 ...
 * 3. It returns the correct JSON response with variant identifier "163893398"
...
describe('REGRESSION: VRTX-0560 - API health check endpoint /api/healthz-smoke-bugfix3-163893398', () => {
  ...
  it('returns JSON response with ok=true and variant=163893398', async () => {
    const res = await GET();
    const json = await res.json();
    expect(json.ok).toBe(true);
    expect(json.variant).toBe('163893398');
  });

  it('returns exactly {"ok":true,"variant":"163893398"} with no extra fields', async () => {
    const res = await GET();
    const json = await res.json();
