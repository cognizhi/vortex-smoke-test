/**
 * Regression Test: VRTX-0553 - /api/healthz-smoke-bugfix2-813098132 returns 404 instead of 200
 *
 * Bug: API health check endpoint returns 404 instead of 200 with correct JSON response.
 * Root Cause: Missing route handler file at src/app/api/healthz-smoke-bugfix2-813098132/route.ts
 ...
 * 3. It returns the correct JSON response with variant identifier "813098132"
...
import { GET } from '../../app/api/healthz-smoke-bugfix2-813098132/route';

describe('REGRESSION: VRTX-0553 - API health check endpoint /api/healthz-smoke-bugfix2-813098132', () => {
  ...
  it('returns JSON response with ok=true and variant=813098132', async () => {
    const res = await GET();
    const json = await res.json();
    expect(json.ok).toBe(true);
    expect(json.variant).toBe('813098132');
  });

  it('returns exactly {"ok":true,"variant":"813098132"} with no extra fields', async () => {
    const res = await GET();
    const json = await res.json();
