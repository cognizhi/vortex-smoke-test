/**
 * Regression Test: VRTX-0474 - /api/healthz-smoke-bugfix-ha3-595526761 returns 404 instead of 200
 *
 ...
 * 3. It returns the correct JSON response with variant identifier "595526761"
...
describe('REGRESSION: VRTX-0474 - API health check endpoint /api/healthz-smoke-bugfix-ha3-595526761', () => {
  it('endpoint exists and responds to ha3-595526761 variant', async () => {
    const res = await getHa3595526761();
    expect(res).toBeDefined();
    expect(res.status).toBe(200);
  });

  it('returns 200 OK for /api/healthz-smoke-bugfix-ha3-595526761', async () => {
    const res = await getHa3595526761();
    const json = await res.json();
    expect(res.status).toBe(200);
    expect(json.ok).toBe(true);
    expect(json.variant).toBe('595526761');
  });

  it('returns exactly {"ok":true,"variant":"595526761"} for ha3-595526761 variant', async () => {
    const res = await getHa3595526761();
    const json = await res.json();
  ...
  it('returns 200 under load (multiple concurrent calls)', async () => {
    const calls = Array.from({ length: 10 }, () => getHa3595526761());
