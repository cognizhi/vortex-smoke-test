/**
 * Regression Test: VRTX-0546 - /api/healthz-smoke-bugfix-261077566 returns 404 instead of 200
 *
 * Bug: API health check endpoint returns 404 instead of 200 with correct JSON response.
 * Root Cause: Missing route handler file at src/app/api/healthz-smoke-bugfix-261077566/route.ts
 ...
 * 3. It returns the correct JSON response with variant identifier "261077566"
...
describe('REGRESSION: VRTX-0546 - API health check endpoint /api/healthz-smoke-bugfix-261077566', () => {
  ...
  it('returns JSON response with ok=true and variant=261077566', async () => {
    const res = await GET();
    const json = await res.json();
    expect(json.ok).toBe(true);
    expect(json.variant).toBe('261077566');
  });

  it('returns exactly {"ok":true,"variant":"261077566"} with no extra fields', async () => {
    const res = await GET();
    const json = await res.json();
