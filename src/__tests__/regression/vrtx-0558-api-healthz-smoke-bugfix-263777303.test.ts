/**
 * Regression Test: VRTX-0558 - /api/healthz-smoke-bugfix-263777303 returns 404 instead of 200
 *
 * Bug: API health check endpoint returns 404 instead of 200 with correct JSON response.
 * Root Cause: Missing route handler file at src/app/api/healthz-smoke-bugfix-263777303/route.ts
 ...
 * 3. It returns the correct JSON response with variant identifier "263777303"
...
import { GET } from '../../app/api/healthz-smoke-bugfix-263777303/route';

describe('REGRESSION: VRTX-0558 - API health check endpoint /api/healthz-smoke-bugfix-263777303', () => {
  ...
  it('returns JSON response with ok=true and variant=263777303', async () => {
    const res = await GET();
    const json = await res.json();
    expect(json.ok).toBe(true);
    expect(json.variant).toBe('263777303');
  });

  it('returns exactly {"ok":true,"variant":"263777303"} with no extra fields', async () => {
    const res = await GET();
    const json = await res.json();
