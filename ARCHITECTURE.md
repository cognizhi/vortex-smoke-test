...
- **`/api/healthz-smoke-{variant}`** (SPRINT-0005+) — Variant-specific health check
  endpoints for deployment verification and A/B testing. Endpoints use a unified
  response envelope: `{ data: { ok: true, variant: "{variant-id}" }, error: null }`
  with zero dependencies. Used by monitoring systems to verify specific application
  variants are deployed and reachable. Current variants: `1072853191` (SPRINT-0017),
  `305070125` (SPRINT-0015), `110428092` (SPRINT-0013), `48842051` (SPRINT-0009),
...
**Added:**
- Variant-specific health check endpoint `/api/healthz-smoke-cancel-1072853191` for deployment
  verification and monitoring. Returns `{ data: { ok: true, variant: "1072853191" }, error: null }`
  with zero dependencies (no database, auth, or external calls).
...
- Comprehensive test coverage for variant endpoint including response validation and performance metrics.

**Implementation details:**
- Follows the lightweight, dependency-free pattern established by previous variant endpoints.
- Implemented as separate route file (`/api/healthz-smoke-cancel-1072853191/route.ts`).
- Response envelope unified with other endpoints: `{ data: { ok: true, variant: "..." }, error: null }`.
