# VRTX-0474: Add Missing Health Check Endpoint /api/healthz-smoke-bugfix-ha3-595526761

## Issue Summary
GET `/api/healthz-smoke-bugfix-ha3-595526761` returns HTTP 404 Not Found.
Expected: HTTP 200 OK with JSON body `{"ok":true,"variant":"595526761"}`

## Root Cause Analysis

**Issue:** The health check endpoint file is missing from the codebase.

**Location:** `/src/app/api/healthz-smoke-bugfix-ha3-595526761/route.ts` does not exist.
...
- **Path:** `/api/healthz-smoke-bugfix-ha3-595526761`
- **Method:** GET
- **Authentication:** None (public endpoint)
- **External Dependencies:** None
- **Database Access:** None
- **Response Status:** 200 OK
- **Response Body:** `{"ok":true,"variant":"595526761"}`
...
```bash
mkdir -p /src/app/api/healthz-smoke-bugfix-ha3-595526761
```

#### Step 2: Create route.ts File
Create `/src/app/api/healthz-smoke-bugfix-ha3-595526761/route.ts` with the following pattern:

**Pattern Reference:** `/src/app/api/healthz-smoke-bugfix-ha-30297400/route.ts` (existing health check endpoint from SPRINT-0082)

**Implementation Requirements:**
1. Import `NextResponse` from `'next/server'`
2. Export async GET function
3. Return `NextResponse.json({ ok: true, variant: "595526761" }, { status: 200 })`
...
/**
 * GET /api/healthz-smoke-bugfix-ha3-595526761
 *
 ...
 * to verify this specific application variant (595526761) is deployed and reachable.
 *
...
- [ ] Response body contains `"variant": "595526761"`
...
### File Changes
- **New File:** `/src/app/api/healthz-smoke-bugfix-ha3-595526761/route.ts`
...
## Acceptance Criteria
- [ ] File `/src/app/api/healthz-smoke-bugfix-ha3-595526761/route.ts` exists
- [ ] File follows Next.js API route pattern with async GET handler
- [ ] Handler returns `NextResponse.json({ ok: true, variant: "595526761" }, { status: 200 })`
- [ ] No authentication or authorization checks
- [ ] No database queries or external service calls
- [ ] Comprehensive JSDoc comments included
- [ ] Regression test assertions validate exact JSON shape `{"ok":true,"variant":"595526761"}`
- [ ] `curl -i http://localhost:3000/api/healthz-smoke-bugfix-ha3-595526761` returns HTTP 200 with correct JSON
...
5. Regression test created and passing to assert exact JSON shape
6. Changes are committed on the sprint branch
7. No test or documentation files need updates (endpoints require no testing beyond curl verification and regression tests)
