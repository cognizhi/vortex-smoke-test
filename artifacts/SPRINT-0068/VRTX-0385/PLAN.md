# VRTX-0385 Implementation Plan

## Ticket Summary

**Title:** Add missing `/healthz-smoke-bugfix2-156326201` endpoint  
**Type:** DEFECT / Fix  
**Sprint:** SPRINT-0068  
...
The endpoint `GET /healthz-smoke-bugfix2-156326201` currently returns **HTTP 404 Not Found**. This endpoint should exist and return a lightweight health check response for smoke testing and monitoring systems.

### Expected Behavior
```
GET /healthz-smoke-bugfix2-156326201 HTTP/1.1

HTTP/1.1 200 OK
Content-Type: application/json

{"ok":true,"variant":"156326201"}
```
...
**File:** `src/app/api/healthz-smoke-bugfix2-156326201/route.ts`

**Content:**
```typescript
/**
 * GET /api/healthz-smoke-bugfix2-156326201
 *
 * Variant-specific lightweight smoke test endpoint for load balancers and monitoring systems.
 * This endpoint identifies the specific variant build (156326201) in the response.
 *
...
/**
 * GET handler for /api/healthz-smoke-bugfix2-156326201
 *
 ...
 *
 * @returns NextResponse with status 200 and body { ok: true, variant: "156326201" }
 */
...
- Similar endpoints with the `bugfix2` naming and numeric variant identifier

---

## Definition of Done

- [ ] Directory `src/app/api/healthz-smoke-bugfix2-156326201/` created
- [ ] File `src/app/api/healthz-smoke-bugfix2-156326201/route.ts` created with correct handler
- [ ] Handler exports async `GET()` function
- [ ] GET handler returns `NextResponse.json()` with `{ ok: true, variant: "156326201" }`
...
- [ ] Endpoint tested: `curl http://localhost:3000/healthz-smoke-bugfix2-156326201` returns 200 + correct JSON
...
- **Existing pattern:** `src/app/api/healthz-smoke-bugfix2-407985318/route.ts`
