...
Add a new, lightweight health check endpoint `/api/healthz-smoke-1026761837-c` to enable deployment verification and variant-specific monitoring. This endpoint is completely independent with no dependencies on database, authentication, or external services.
...
Endpoint suffix: `c` (three total: a, b, c)
...
**1. Route Handler:**
- **Path:** `src/app/api/healthz-smoke-1026761837-c/route.ts`
...
**2. Test File:**
- **Path:** `src/app/api/healthz-smoke-1026761837-c/__tests__/route.test.ts`
...
```typescript
/**
 * GET /api/healthz-smoke-1026761837-c
 *
...
/**
 * GET handler for /api/healthz-smoke-1026761837-c
 *
...
```typescript
/**
 * Unit tests for GET /api/healthz-smoke-1026761837-c
 *
...
describe('GET /api/healthz-smoke-1026761837-c', () => {
  beforeEach(() => {
...
|------|-------|---------|--------|
| `src/app/api/healthz-smoke-1026761837-c/route.ts` | Engineer | Route handler | NEW |
| `src/app/api/healthz-smoke-1026761837-c/__tests__/route.test.ts` | Engineer | Unit tests | NEW |

**No Dependencies:** This task touches only files in `src/app/api/healthz-smoke-1026761837-c/`. No shared modules, no database changes, no config changes.

**Parallel Execution:** Safe to run alongside VRTX-0341 and VRTX-0343 (disjoint file sets).
...
### Implementation
- ✅ Route handler created at `src/app/api/healthz-smoke-1026761837-c/route.ts`
...
### Testing
- ✅ Test file created at `src/app/api/healthz-smoke-1026761837-c/__tests__/route.test.ts`
...
- ✅ All tests pass: `npm run test src/app/api/healthz-smoke-1026761837-c/__tests__/route.test.ts`
...
1. [ ] Code written: `src/app/api/healthz-smoke-1026761837-c/route.ts` with GET handler
2. [ ] Tests written: `src/app/api/healthz-smoke-1026761837-c/__tests__/route.test.ts` with 7 test cases
...
7. [ ] Manual test: HTTP GET to localhost:3000/api/healthz-smoke-1026761837-c returns 200 with correct JSON
...
**Environment:** jsdom  
**File:** `src/app/api/healthz-smoke-1026761837-c/__tests__/route.test.ts`
...
4. **Verify status:**
   ```bash
   curl -i http://localhost:3000/api/healthz-smoke-1026761837-c
...
1. Delete `src/app/api/healthz-smoke-1026761837-c/` directory
2. Revert the commit
