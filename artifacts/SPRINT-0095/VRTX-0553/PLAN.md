# VRTX-0553 Implementation Plan

**Title:** [smoke-bugfix-178459795870584] /healthz-smoke-bugfix2-813098132 returns 404, should return ok+variant
...
GET request to `/api/healthz-smoke-bugfix2-813098132` currently returns HTTP 404 Not Found.
...
The endpoint is missing entirely. The directory `src/app/api/healthz-smoke-bugfix2-813098132/` and its handler file `route.ts` do not exist in the codebase.
...
curl -v http://localhost:3000/api/healthz-smoke-bugfix2-813098132
...
1. **Create the endpoint directory:**
   ```bash
   mkdir -p src/app/api/healthz-smoke-bugfix2-813098132
   ```

2. **Create the handler file:** `src/app/api/healthz-smoke-bugfix2-813098132/route.ts`
   - Pattern: Copy and adapt from `src/app/api/healthz-smoke-bugfix-ha2-244944780/route.ts`
   - Change the variant ID in the response from "244944780" to "813098132"
   - Keep the same JSDoc documentation structure with the correct endpoint path
   - Export async function `GET()` that returns `NextResponse.json({ok:true, variant:"813098132"}, {status:200})`
...
4. **Verification:**
   - Handler is located at correct path: `src/app/api/healthz-smoke-bugfix2-813098132/route.ts`
   - Variant ID in response matches the endpoint name: "813098132"
   - No dependencies (no database, no auth, no external calls)
   - Returns exactly and only: `{"ok":true,"variant":"813098132"}`

### File Structure

```
src/app/api/healthz-smoke-bugfix2-813098132/
└── route.ts
```
...
- **Response Format:** Must match exactly: `{"ok":true,"variant":"813098132"}`
...
- [ ] Directory `src/app/api/healthz-smoke-bugfix2-813098132/` created
- [ ] File `src/app/api/healthz-smoke-bugfix2-813098132/route.ts` created with correct implementation
- [ ] GET request to `/api/healthz-smoke-bugfix2-813098132` returns HTTP 200
- [ ] Response body is exactly: `{"ok":true,"variant":"813098132"}`
...
# Expected: {"ok":true,"variant":"813098132"}

curl -w "\nStatus: %{http_code}\n" http://localhost:3000/api/healthz-smoke-bugfix2-813098132
# Expected status: 200
```
