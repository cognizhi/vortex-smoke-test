...
## Issue
GET `/api/healthz-smoke-bugfix-254027906` returned 404 instead of 200 with response `{"ok":true,"variant":"254027906"}`.
...
1. **`src/app/api/healthz-smoke-bugfix-254027906/route.ts`** — Route handler
   - Implements GET handler that returns `{"ok":true,"variant":"254027906"}` with 200 status
...
2. **`src/app/api/healthz-smoke-bugfix-254027906/__tests__/route.test.ts`** — Regression test
...
- **PASS (GREEN)** after this fix: HTTP 200 with correct JSON `{"ok":true,"variant":"254027906"}`
