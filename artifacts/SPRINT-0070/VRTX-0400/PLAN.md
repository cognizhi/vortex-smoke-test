# VRTX-0400: Implement /healthz-smoke-1012136249-a endpoint
...
Create a completely self-contained health check endpoint at `/api/healthz-smoke-1012136249-a` that returns variant-specific health status with zero dependencies (no database, auth, or external calls).
...
### Files to Create
- `src/app/api/healthz-smoke-1012136249-a/route.ts` — GET handler
- `src/app/api/healthz-smoke-1012136249-a/__tests__/route.test.ts` — Test suite (15 tests)

### No Shared Code
- This implementation is completely independent; no shared utilities or helpers with endpoints B or C
- Pattern taken directly from SPRINT-0069 endpoint `/api/healthz-smoke-276127630-a`
...
### Handler Template (from SPRINT-0069 variant a)
...
6. **Manual verification** — `curl http://localhost:3000/api/healthz-smoke-1012136249-a` returns correct JSON
...
```typescript
// src/app/api/healthz-smoke-1012136249-a/__tests__/route.test.ts
import { describe, it, expect } from 'vitest';
import { GET } from '../route';

describe('GET /api/healthz-smoke-1012136249-a', () => {
...
- This endpoint is **completely independent** — no shared helpers with endpoints B or C
- Copy-paste implementation from SPRINT-0069-a if desired for consistency
