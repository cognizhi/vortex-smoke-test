# VRTX-XXXX2 Plan: Implement Endpoint B (`/healthz-smoke-661868846-b`)

**TASK:** Implement the second independent health check variant endpoint
...
Implement the GET endpoint `/api/healthz-smoke-661868846-b` that returns `{ ok: true, variant: "661868846" }` with HTTP 200. This endpoint is **fully standalone** — no shared code with endpoints A or C, no dependencies, no database access.
...
**Implementation:**
- Single route file: `src/app/api/healthz-smoke-661868846-b/route.ts`
...
**Testing:**
- Unit test file: `src/app/api/healthz-smoke-661868846-b/__tests__/route.test.ts`
...
- Zero dependencies on TASK-1 or TASK-3
...
- [ ] Create directory `src/app/api/healthz-smoke-661868846-b/`
...
Pattern from previous sprints (509572604-b, 53261999-b, etc.):
...
describe('GET /api/healthz-smoke-661868846-b', () => {
  it('returns 200 with correct JSON', async () => {
    const request = new NextRequest('http://localhost:3000/api/healthz-smoke-661868846-b', {
      method: 'GET',
    });
...
- SPRINT-0093 (929192825-b) — Three independent endpoints
- SPRINT-0092 (509572604-b) — Three independent endpoints with E2E tests
- SPRINT-0088 (53261999-b) — Three independent endpoints
- SPRINT-0073 (121996100-b) — Pattern established
- SPRINT-0070 (1012136249-b), SPRINT-0069 (276127630-b), and earlier variants

**References:**
- Implementation reference: `/src/app/api/healthz-smoke-509572604-b/route.ts`
- Test reference: `/src/app/api/healthz-smoke-509572604-b/__tests__/route.test.ts`
...
- [ ] Endpoint is registered at `/api/healthz-smoke-661868846-b`
...
1. **No Shared Code:** This endpoint must have zero code reuse with VRTX-XXXX1 or VRTX-XXXX3. Each is a completely independent module.
