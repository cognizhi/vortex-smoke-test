...
Implement a lightweight, self-contained health check endpoint for variant 778162394 (variant c path), following the established pattern from SPRINT-0029 (VRTX-0232).

This is the third endpoint variant (path "-c") for the same variant identifier.

## Requirement

**What:** Health check endpoint at `/api/healthz-smoke-778162394-c`
...
- [ ] Write test suite at `/src/app/api/healthz-smoke-778162394-c/__tests__/route.test.ts` (red)
- [ ] Run tests to verify they fail (confirm red phase)
- [ ] Implement handler at `/src/app/api/healthz-smoke-778162394-c/route.ts` (green)
...
### Files Created
1. `/src/app/api/healthz-smoke-778162394-c/route.ts`
   - GET handler returning hardcoded response with variant "778162394"
   - JSDoc comments matching pattern

2. `/src/app/api/healthz-smoke-778162394-c/__tests__/route.test.ts`
...
- Route path: `healthz-smoke-572185676` → `healthz-smoke-778162394-c`
...
- This is endpoint variant "-c" for the 778162394 variant identifier
