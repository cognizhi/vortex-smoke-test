# TASK VRTX-0305 Plan: Implement /healthz-smoke-282954433-a endpoint
...
Implement the `/api/healthz-smoke-282954433-a` health check endpoint — a lightweight, self-contained smoke test endpoint for variant 282954433, path A.

**Variant ID:** 282954433  
**Endpoint path:** `/api/healthz-smoke-282954433-a`
...
### Request
```
GET /api/healthz-smoke-282954433-a
```
...
- **`src/app/api/healthz-smoke-282954433-a/route.ts`** (new)
  - GET handler returning fixed JSON response
  - No dependencies (no DB, no auth, no env vars)
  - ~40 lines

### Test files:
- **`src/app/api/healthz-smoke-282954433-a/__tests__/route.test.ts`** (new)
...
/**
 * GET handler for /api/healthz-smoke-282954433-a
 *
...
describe('GET /api/healthz-smoke-282954433-a', () => {
...
- `curl http://localhost:3000/api/healthz-smoke-282954433-a`
- Verify HTTP 200 and response matches spec

### CI verification:
- `npm run test -- src/app/api/healthz-smoke-282954433-a/` — all tests passing
...
- Route file created: `src/app/api/healthz-smoke-282954433-a/route.ts`
...
✅ **Tests comprehensive:**
- Test file created: `src/app/api/healthz-smoke-282954433-a/__tests__/route.test.ts`
...
- This endpoint is completely independent from endpoints B and C
- No shared code, no dependencies between implementations
- Can be implemented in parallel with VRTX-0306 and VRTX-0307
