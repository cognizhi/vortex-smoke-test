# VRTX-0559 Implementation Plan

**Title:** [smoke-bugfix-178460008986121] /healthz-smoke-bugfix2-589426407 returns 404, should return ok+variant
...
GET request to `/api/healthz-smoke-bugfix2-589426407` currently returns HTTP 404 Not Found.
...
The endpoint is missing entirely. The directory `src/app/api/healthz-smoke-bugfix2-589426407/` and its handler file `route.ts` do not exist in the codebase.

This endpoint follows a well-established pattern used by other variant-specific health check endpoints (e.g., `healthz-smoke-bugfix2-407985318`), but the implementation for this variant ID was not created.
...
curl -v http://localhost:3000/api/healthz-smoke-bugfix2-589426407
...
1. **Create the endpoint directory:**
   ```bash
   mkdir -p src/app/api/healthz-smoke-bugfix2-589426407
   ```

2. **Create the handler file:** `src/app/api/healthz-smoke-bugfix2-589426407/route.ts`
   - Pattern: Copy and adapt from `src/app/api/healthz-smoke-bugfix2-407985318/route.ts`
   - Change the variant ID in the response from "407985318" to "589426407"
   - Keep the same JSDoc documentation structure with the correct endpoint path
   - Export async function `GET()` that returns `NextResponse.json({ok:true, variant:"589426407"}, {status:200})`
...
4. **Verification:**
   - Handler is located at correct path: `src/app/api/healthz-smoke-bugfix2-589426407/route.ts`
   - Variant ID in response matches the endpoint name: "589426407"
   - No dependencies (no database, no auth, no external calls)
   - Returns exactly and only: `{"ok":true,"variant":"589426407"}`

### File Structure

```
src/app/api/healthz-smoke-bugfix2-589426407/
└── route.ts
```
...
- **Response Format:** Must match exactly: `{"ok":true,"variant":"589426407"}`
...
- [ ] Directory `src/app/api/healthz-smoke-bugfix2-589426407/` created
- [ ] File `src/app/api/healthz-smoke-bugfix2-589426407/route.ts` created with correct implementation
- [ ] GET request to `/api/healthz-smoke-bugfix2-589426407` returns HTTP 200
- [ ] Response body is exactly: `{"ok":true,"variant":"589426407"}`
...
- [ ] Code follows existing pattern from `src/app/api/healthz-smoke-bugfix2-407985318/route.ts`
...
# Test the endpoint
curl http://localhost:3000/api/healthz-smoke-bugfix2-589426407
# Expected: {"ok":true,"variant":"589426407"}

curl -w "\nStatus: %{http_code}\n" http://localhost:3000/api/healthz-smoke-bugfix2-589426407
# Expected status: 200
```
...
- Exact pattern already exists and is proven (`healthz-smoke-bugfix2-407985318`)
...
- **Existing Variant Endpoint Pattern:** `src/app/api/healthz-smoke-bugfix2-407985318/route.ts`
