...
The health check endpoint `GET /healthz-smoke-bugfix2-927673095` is missing from the codebase and returns HTTP 404. This endpoint should return a lightweight, self-contained variant-specific health check response for deployment verification and monitoring systems.

**Expected behavior:** The endpoint should respond with HTTP 200 and the JSON body `{ "ok": true, "variant": "927673095" }`.
...
2. Send a GET request: `curl http://localhost:3000/healthz-smoke-bugfix2-927673095`
3. **Expected:** HTTP 200 with response body `{ "ok": true, "variant": "927673095" }`
...
**Root cause location:** File `/workspace/repo/src/app/api/healthz-smoke-bugfix2-927673095/route.ts` does not exist.
...
1. **Create route handler file:** `src/app/api/healthz-smoke-bugfix2-927673095/route.ts`
   - Export an async `GET()` handler function
   - Return `NextResponse.json({ ok: true, variant: "927673095" }, { status: 200 })`
   - Add JSDoc header documenting the endpoint (no auth, no dependencies, fast response)

2. **Create test file:** `src/app/api/healthz-smoke-bugfix2-927673095/__tests__/route.test.ts`
   - Test HTTP 200 status code
   - Test JSON response structure: `{ ok: true, variant: "927673095" }`
...
- It follows the established pattern from 10+ existing variant endpoints
...
- **FIX-01:** `curl http://localhost:3000/healthz-smoke-bugfix2-927673095` returns HTTP 200 with body `{ "ok": true, "variant": "927673095" }`.
...
- **FIX-05:** Response body has exactly two fields: `ok` (boolean true) and `variant` (string "927673095").
...
Write 14 test cases in `src/app/api/healthz-smoke-bugfix2-927673095/__tests__/route.test.ts` that verify:
...
- `variant` field is string "927673095" (not number)
...
/**
 * GET /healthz-smoke-bugfix2-927673095
 *
 ...
 *
 * @returns NextResponse with status 200 and body { ok: true, variant: "927673095" }
 */
