# Bug Specification: Missing /api/healthz-smoke-1024087252 Endpoint
...
The endpoint `/api/healthz-smoke-1024087252` returns HTTP 404 (Not Found) when accessed.

**Expected behavior:** The endpoint should respond with HTTP 200 and a JSON body containing `{ "ok": true, "variant": "1024087252" }`.

**Impact:** Monitoring systems and load balancers cannot verify that this specific variant (1024087252) is deployed and reachable, breaking deployment verification for canary deployments and A/B testing scenarios.
...
2. **Action:** Make a GET request to `/api/healthz-smoke-1024087252`
   ```bash
...
**Root cause:** The endpoint handler file `/src/app/api/healthz-smoke-1024087252/route.ts` does not exist.

The codebase has established a pattern for variant-specific health check endpoints:
...
- And others documented in PRODUCT.md section 8 and ARCHITECTURE.md section 5

Each endpoint is implemented as a separate route handler following Next.js App Router convention.

**Missing file:** `src/app/api/healthz-smoke-1024087252/route.ts`
**Missing test file:** `src/app/api/healthz-smoke-1024087252/__tests__/route.test.ts`
...
### File: `src/app/api/healthz-smoke-1024087252/route.ts`
...
- Matches the exact pattern used in all existing variant endpoints (e.g., `/api/healthz-smoke-110428092/route.ts`)
...
2. **Field Type Safety** (2 tests)
   ...
   - `variant` field is string `"1024087252"` (not number)
...
- **FIX-01:** GET `/api/healthz-smoke-1024087252` returns HTTP 200 status
- **FIX-02:** Response body is exactly `{ "ok": true, "variant": "1024087252" }` with no extra fields
- **FIX-03:** Content-Type header is `application/json`
- **FIX-04:** `ok` field is boolean `true` (not just truthy)
- **FIX-05:** `variant` field is string `"1024087252"` (not number)
...
- ✅ Field types (boolean true, string "1024087252")
...
- [ ] Create `__tests__/route.test.ts` with 14+ test cases
- [ ] All tests pass (red → green)
- [ ] Lint passes: `npm run lint` (0 warnings)
- [ ] Type check passes: `npm run typecheck`
- [ ] Linter: ESLint rules pass
...
- **Existing implementations:**
  ...
  - `/src/app/api/healthz-smoke-48842051/route.ts` (SPRINT-0009)
