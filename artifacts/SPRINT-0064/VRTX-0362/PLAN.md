...
**Scope:** Implement a single, self-contained variant-specific health check endpoint at `/api/healthz-smoke-637917955-b` with comprehensive test coverage.

---

## 1. What We're Building

A lightweight, dependency-free HTTP GET endpoint that returns a JSON response identifying the "637917955-b" variant.
...
### Handler File: `src/app/api/healthz-smoke-637917955-b/route.ts`
...
/**
 * GET handler for /api/healthz-smoke-637917955-b
 *
...
describe('GET /api/healthz-smoke-637917955-b', () => {
  beforeEach(() => {
    // No setup needed
  });
...
- [ ] File `src/app/api/healthz-smoke-637917955-b/route.ts` exists and is committed
- [ ] File `src/app/api/healthz-smoke-637917955-b/__tests__/route.test.ts` exists and is committed
- [ ] All 15 tests pass: `npm run test src/app/api/healthz-smoke-637917955-b/__tests__/route.test.ts`
...
- [ ] Manual verification: `GET http://localhost:3000/api/healthz-smoke-637917955-b` returns `{"ok":true,"variant":"637917955"}`
...
```bash
# Single test file
npm run test src/app/api/healthz-smoke-637917955-b/__tests__/route.test.ts

# Watch mode
npm run test -- --watch src/app/api/healthz-smoke-637917955-b/__tests__/route.test.ts

# Coverage report
npm run test:coverage src/app/api/healthz-smoke-637917955-b/__tests__/route.test.ts
```
...
1. **Copy-paste OK:** You can copy the exact pattern from `src/app/api/healthz-smoke-28611693/` as a template, then update:
   - The directory name to `healthz-smoke-637917955-b`
   - The variant string to `"637917955"` (same in all three endpoints)
   - The endpoint path in comments to `/api/healthz-smoke-637917955-b`
...
- ✅ Endpoint accessible at `GET /api/healthz-smoke-637917955-b`
