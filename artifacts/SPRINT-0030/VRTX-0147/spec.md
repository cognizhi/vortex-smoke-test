# Bug Specification: Missing /api/healthz-smoke-bugfix2-446144862 Endpoint
...
The variant-specific health check endpoint `/api/healthz-smoke-bugfix2-446144862` is missing from the application. Attempts to access it return HTTP 404, but the endpoint should exist and respond with a lightweight health check response.

This endpoint is part of the established variant smoke test endpoint pattern documented in PRODUCT.md and ARCHITECTURE.md. Similar endpoints (e.g., `/api/healthz-smoke-bugfix2-555866324`, `/api/healthz-smoke-bugfix2-1047318619`, etc.) already exist and are in production use for deployment verification and monitoring.

## 2. Reproduction Steps

1. Start the development server: `npm run dev`
2. Make a GET request to `/api/healthz-smoke-bugfix2-446144862`
3. **Expected**: HTTP 200 with JSON response: `{"ok":true,"variant":"446144862"}`
...
- **Missing file**: `src/app/api/healthz-smoke-bugfix2-446144862/route.ts`
- **Missing test file**: `src/app/api/healthz-smoke-bugfix2-446144862/__tests__/route.test.ts`
...
**Root cause**: The route handler for this variant was not created when the bug was introduced or when the variant was requested. Existing similar endpoints follow the same pattern and work correctly (examples: `/api/healthz-smoke-bugfix2-555866324` at `src/app/api/healthz-smoke-bugfix2-555866324/route.ts`).
...
**Path**: `src/app/api/healthz-smoke-bugfix2-446144862/route.ts`

- Export async `GET` function returning `NextResponse`
- Response body: `{ ok: true, variant: "446144862" }` with status 200
...
**Path**: `src/app/api/healthz-smoke-bugfix2-446144862/__tests__/route.test.ts`
...
|------|------|------------|
| Other health endpoints | Low | New endpoint is independent; uses same pattern as 10+ existing variants |
...
- **FIX-01**: GET `/api/healthz-smoke-bugfix2-446144862` returns HTTP 200 status
- **FIX-02**: Response body is valid JSON: `{"ok":true,"variant":"446144862"}`
...
| TC-003 | `variant` field value | `"446144862"` |
...
| Additional | Exact shape match | `{ ok: true, variant: "446144862" }` |
...
### Integration Check
- Verify endpoint is reachable via `curl`: `curl http://localhost:3000/api/healthz-smoke-bugfix2-446144862`
...
- [ ] Test file has 18+ comprehensive test cases
...
- [ ] Follows the exact pattern from `src/app/api/healthz-smoke-bugfix2-555866324/`
