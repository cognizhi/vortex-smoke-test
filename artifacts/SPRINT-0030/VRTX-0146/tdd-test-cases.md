# TDD Test Cases: Missing /api/healthz-smoke-bugfix-240218546 Endpoint
...
| TC-003 | Unit | Value | `variant` field is string "240218546" | ✓ variant === "240218546" | route.test.ts |
...
| Additional-02 | Unit | Shape | Exact response shape match | ✓ === { ok: true, variant: "240218546" } | route.test.ts |
...
### Red Phase (Step 5)
```bash
npx vitest run src/app/api/healthz-smoke-bugfix-240218546/__tests__/route.test.ts --reporter=verbose
```

**Expected outcome**: All 21 tests FAIL with `Cannot find module '../route'` or similar, because:
- File `src/app/api/healthz-smoke-bugfix-240218546/route.ts` does not exist yet
...
### Green Phase (Step 11)
After implementing the route handler at `src/app/api/healthz-smoke-bugfix-240218546/route.ts`:

```bash
npx vitest run src/app/api/healthz-smoke-bugfix-240218546/__tests__/route.test.ts
```
...
1. **Test file location**: `src/app/api/healthz-smoke-bugfix-240218546/__tests__/route.test.ts`
2. **Handler location**: `src/app/api/healthz-smoke-bugfix-240218546/route.ts`
3. **Pattern reference**: Mirror `src/app/api/healthz-smoke-bugfix-630670662/` exactly
4. **Variant identifier**: Must be hardcoded as `"240218546"` (not dynamic)
