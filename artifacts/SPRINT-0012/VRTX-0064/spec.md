# Bug Specification: Missing /api/healthz-smoke-bugfix2-555866324 Endpoint
...
The variant-specific health check endpoint `/api/healthz-smoke-bugfix2-555866324` is missing and returns HTTP 404. This endpoint is required for deployment verification and monitoring system integration during the smoke-bugfix-178316046470767 deployment.

**Expected behavior:** GET request to `/api/healthz-smoke-bugfix2-555866324` should return HTTP 200 with JSON body `{"ok":true,"variant":"555866324"}`.
...
2. Open terminal and run: `curl http://localhost:3000/api/healthz-smoke-bugfix2-555866324`
3. Expected response:
   ```json
...
**Root cause:** Missing file `src/app/api/healthz-smoke-bugfix2-555866324/route.ts` — endpoint does not exist.
...
2. Add route handler: `src/app/api/healthz-smoke-bugfix2-555866324/route.ts`
...
3. Create test file: `src/app/api/healthz-smoke-bugfix2-555866324/__tests__/route.test.ts`
   - Verify HTTP 200 status
   - Verify exact response shape `{ ok: true, variant: "555866324" }`
...
- **FIX-01**: Endpoint `GET /api/healthz-smoke-bugfix2-555866324` responds with HTTP 200
- **FIX-02**: Response body is exactly `{"ok":true,"variant":"555866324"}` (no extra fields)
...
- **Status & Response Shape** (TC-001 to TC-008)
  ...
  - Exact field values (ok=true, variant="555866324")
  - No extra fields
