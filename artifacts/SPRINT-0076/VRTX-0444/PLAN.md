# VRTX-0444 Fix Plan — `/api/healthz-smoke-bugfix-582647444` Missing Endpoint

## Problem Statement

The endpoint `GET /api/healthz-smoke-bugfix-582647444` currently returns HTTP 404.

**Expected behavior**: Return HTTP 200 with JSON response `{ "ok": true, "variant": "582647444" }`
...
- Create one new file: `src/app/api/healthz-smoke-bugfix-582647444/route.ts`
...
```bash
mkdir -p src/app/api/healthz-smoke-bugfix-582647444
```

### Step 2: Create Route File
Create `src/app/api/healthz-smoke-bugfix-582647444/route.ts` with the following content:
 ...
 * This endpoint identifies the specific variant build (582647444) in the response.
 *
 ...
 *
 * Response body:
 *   { "ok": true, "variant": "582647444" }
 */
import { NextResponse } from 'next/server';

/**
 * GET handler for /api/healthz-smoke-bugfix-582647444
 *
 ...
 *
 * @returns NextResponse with status 200 and body { ok: true, variant: "582647444" }
 */
...
- [x] Route file created at `src/app/api/healthz-smoke-bugfix-582647444/route.ts`
- [x] File exports an async `GET()` function
- [x] GET handler returns NextResponse.json with status 200
- [x] Response body matches contract: `{ "ok": true, "variant": "582647444" }`
...
- [x] Endpoint responds to `GET http://localhost:3000/api/healthz-smoke-bugfix-582647444`
- [x] Response status is 200 OK
- [x] Response body is valid JSON: `{"ok":true,"variant":"582647444"}`
...
# Expected output: {"ok":true,"variant":"582647444"}

# Verify response code
curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/api/healthz-smoke-bugfix-582647444
# Expected output: 200
```
...
- VRTX-0445: Similar fix for `/api/healthz-smoke-bugfix2-887319380` (independent, can be done in parallel)
