# TASK-0390 Plan — Implement /healthz-smoke-276127630-a endpoint

**Sprint**: SPRINT-0069  
**Task**: Implement `/healthz-smoke-276127630-a` GET endpoint and unit tests  
**Variant**: 276127630 (identifier "a")
...
```
src/app/api/healthz-smoke-276127630-a/
...
### Handler (`src/app/api/healthz-smoke-276127630-a/route.ts`)
...
### Tests (`src/app/api/healthz-smoke-276127630-a/__tests__/route.test.ts`)
...
3. **Response Structure** — Verify `ok` and `variant` fields exist
   ```typescript
...
describe('/api/healthz-smoke-276127630-a', () => {
  let mockRequest: NextRequest

  beforeEach(() => {
    mockRequest = new NextRequest(
      new URL('http://localhost:3000/api/healthz-smoke-276127630-a')
    )
  })
...
This task **owns** only:
- `src/app/api/healthz-smoke-276127630-a/route.ts`
- `src/app/api/healthz-smoke-276127630-a/__tests__/route.test.ts`
...
✅ Route handler file created at `src/app/api/healthz-smoke-276127630-a/route.ts`  
...
✅ `npm run test -- src/app/api/healthz-smoke-276127630-a` passes  
...
**Request**: GET `/api/healthz-smoke-276127630-a` (no body, no auth)
...
- **Parallel**: Can be developed in parallel with VRTX-0391 and VRTX-0392.

---

## Success Metrics

- Endpoint reachable at `GET /api/healthz-smoke-276127630-a`
- Response status: 200
