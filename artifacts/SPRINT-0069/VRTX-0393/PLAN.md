# TASK-0393 Plan — Implement /healthz-smoke-276127630-b endpoint

**Sprint**: SPRINT-0069  
**Task**: VRTX-0393 — Implement `/healthz-smoke-276127630-b` GET endpoint and unit tests  
**Variant**: 276127630 (identifier "b")
...
**Path**: `/api/healthz-smoke-276127630-b`  
**Method**: GET  
**Response (200 OK)**:
```json
{
...
```
src/app/api/healthz-smoke-276127630-b/
...
### Handler (`src/app/api/healthz-smoke-276127630-b/route.ts`)
...
### Tests (`src/app/api/healthz-smoke-276127630-b/__tests__/route.test.ts`)
...
**Test File Template** (identical to VRTX-0390 except path):
```typescript
import { describe, it, expect, beforeEach } from 'vitest'
import { GET } from '../route'
import { NextRequest } from 'next/server'

describe('/api/healthz-smoke-276127630-b', () => {
  let mockRequest: NextRequest

  beforeEach(() => {
    mockRequest = new NextRequest(
      new URL('http://localhost:3000/api/healthz-smoke-276127630-b')
    )
  })
...
This task **owns** only:
- `src/app/api/healthz-smoke-276127630-b/route.ts`
- `src/app/api/healthz-smoke-276127630-b/__tests__/route.test.ts`
...
✅ Route handler file created at `src/app/api/healthz-smoke-276127630-b/route.ts`  
✅ Test file created at `src/app/api/healthz-smoke-276127630-b/__tests__/route.test.ts`  
✅ All 7 unit tests passing (100% coverage for this endpoint)  
✅ `npm run typecheck` passes with no errors  
✅ `npm run lint` passes with 0 warnings (ESLint on these files)  
✅ `npm run test -- src/app/api/healthz-smoke-276127630-b` passes  
...
**Request**: GET `/api/healthz-smoke-276127630-b` (no body, no auth)
...
- **Parallel**: Can be developed in parallel with VRTX-0392 and VRTX-0394.

---

## Success Metrics

- Endpoint reachable at `GET /api/healthz-smoke-276127630-b`
- Response status: 200
