...
**Endpoint:** `GET /api/healthz-smoke-509572604-c`  
...
### File Location
```
src/app/api/healthz-smoke-509572604-c/
  └── route.ts
```
...
1. **No Shared Code** — This endpoint is completely independent from endpoints -a and -b. Duplication is intentional and acceptable because:
...
curl http://localhost:3000/api/healthz-smoke-509572604-c
...
- ✅ No shared code with endpoints -a or -b (no helpers, no utils)
- ✅ TypeScript strict mode compliance (npm run typecheck passes)
- ✅ ESLint passes (npm run lint passes, 0 warnings)
- ✅ Manual test: `curl http://localhost:3000/api/healthz-smoke-509572604-c` returns correct JSON
...
**HTTP Interface:**
- **Method:** GET
- **Path:** `/api/healthz-smoke-509572604-c`
- **Status Code:** 200 OK
...
7. ✅ No merge conflicts with other endpoints (-a, -b)
...
1. **Branch:** `vortex/feat/VRTX-0530-implement-endpoint-c`
2. **Commit:** One clear commit with the route file
   ```
   feat(smoke): /api/healthz-smoke-509572604-c endpoint
...
- **Epic:** VRTX-0525
- **Related Tasks:** VRTX-0528 (endpoint -a), VRTX-0529 (endpoint -b)
