...
Implement the first independent GET endpoint for variant 509572604 deployment verification.
...
### File Location
```
src/app/api/healthz-smoke-509572604-a/
  └── route.ts
```
...
1. **No Shared Code** — This endpoint is completely independent from endpoints -b and -c. Duplication is intentional and acceptable because:
...
curl http://localhost:3000/api/healthz-smoke-509572604-a
...
- ✅ No shared code with endpoints -b or -c (no helpers, no utils)
- ✅ TypeScript strict mode compliance (npm run typecheck passes)
- ✅ ESLint passes (npm run lint passes, 0 warnings)
- ✅ Manual test: `curl http://localhost:3000/api/healthz-smoke-509572604-a` returns correct JSON
...
**HTTP Interface:**
- **Method:** GET
- **Path:** `/api/healthz-smoke-509572604-a`
- **Status Code:** 200 OK
...
7. ✅ No merge conflicts with other endpoints (-b, -c)
...
1. **Branch:** `vortex/feat/VRTX-0528-implement-endpoint-a`
2. **Commit:** One clear commit with the route file
   ```
   feat(smoke): /api/healthz-smoke-509572604-a endpoint
...
- **Epic:** VRTX-0525
- **Related Tasks:** VRTX-0529 (endpoint -b), VRTX-0530 (endpoint -c)
