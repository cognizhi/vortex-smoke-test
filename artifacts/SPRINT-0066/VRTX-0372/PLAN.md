# VRTX-0372: Add Missing `/api/healthz-smoke-bugfix2-471601007` Endpoint

**Related:** SPRINT-0066 (see `artifacts/SPRINT-0066/SPRINT-PLAN.md`)

## Summary

Add the missing variant-specific healthz endpoint `/api/healthz-smoke-bugfix2-471601007` that returns a 200 status with variant identification. This endpoint is required by monitoring and smoke test systems for deployment validation.

## Problem

- **Current behavior:** GET `/api/healthz-smoke-bugfix2-471601007` returns 404
- **Expected behavior:** GET `/api/healthz-smoke-bugfix2-471601007` returns 200 with JSON `{ ok: true, variant: "471601007" }`
...
The endpoint directory and route handler do not exist in the codebase. File path `src/app/api/healthz-smoke-bugfix2-471601007/route.ts` is missing.

## Solution

Create a new Next.js API route following the existing variant pattern found in files like `src/app/api/healthz-smoke-bugfix2-446144862/route.ts`.

### Implementation

**Create file:** `src/app/api/healthz-smoke-bugfix2-471601007/route.ts`
 ...
 * This endpoint identifies the specific variant build (471601007) in the response.
 *
...
/**
 * GET handler for /api/healthz-smoke-bugfix2-471601007
 *
 ...
 *
 * @returns NextResponse with status 200 and body { ok: true, variant: "471601007" }
 */
...
- [x] Endpoint is created at `src/app/api/healthz-smoke-bugfix2-471601007/route.ts`
- [x] GET handler returns HTTP 200
- [x] Response body is valid JSON: `{ ok: true, variant: "471601007" }`
...
- [x] Manual test passes: `curl http://localhost:3000/api/healthz-smoke-bugfix2-471601007` returns 200
...
- **New:** `src/app/api/healthz-smoke-bugfix2-471601007/route.ts` (38 lines)
