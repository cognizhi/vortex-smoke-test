...
**Effort:** 30 minutes  
**Phase:** Implementation — Endpoint A

---

## Summary

Implement a lightweight, self-contained GET endpoint at `/api/healthz-smoke-43762983-a` that returns `{ ok: true, variant: "43762983" }` with HTTP 200 status. The endpoint has zero dependencies (no database, auth, or external calls) and is designed for deployment verification and monitoring.

**File ownership:**
- `src/app/api/healthz-smoke-43762983-a/route.ts` — The sole file for this endpoint
...
**In scope:**
- Create route file at `src/app/api/healthz-smoke-43762983-a/route.ts`
...
- Zero dependencies (no imports of helper functions, utilities, middleware)
...
### Route: `GET /api/healthz-smoke-43762983-a`

**Request:**
```
GET /api/healthz-smoke-43762983-a HTTP/1.1
Host: {slug}.{domain}
```
...
### File: `src/app/api/healthz-smoke-43762983-a/route.ts`

**Structure:**
1. Import `NextResponse` from `'next/server'`
2. JSDoc block:
   - Route path: `GET /api/healthz-smoke-43762983-a`
...
**Example implementation pattern (from `/api/healthz-smoke-572185676/route.ts`):**
...
- [ ] Route file `src/app/api/healthz-smoke-43762983-a/route.ts` created
...
- [ ] Endpoint is self-contained (no shared code, no helper imports)
- [ ] File is committed on ticket branch
- [ ] Ready for test harness (VRTX-0332)

---

## Testing Strategy (VRTX-0332)
...
- [ ] Create file at exact path `src/app/api/healthz-smoke-43762983-a/route.ts`
...
- [ ] Commit with clear message: "Add healthz-smoke-43762983-a endpoint for deployment verification"
