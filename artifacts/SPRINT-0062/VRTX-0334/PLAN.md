# VRTX-0332: Test harness for healthz-smoke-43762983-a

**Task type:** Implementation (Tests)  
**Sprint:** SPRINT-0062  
**Epic:** Health check endpoints (variant 43762983)  
**Effort:** 45 minutes  
**Phase:** Test Harness — Endpoint A  
**Depends on:** VRTX-0329 (endpoint A implementation)

---

## Summary

Create comprehensive test suite for the healthz-smoke-43762983-a endpoint. 14 test cases covering correctness, type safety, performance, and public access. All tests must pass with 100% coverage of the GET handler.

**File ownership:**
- `src/app/api/healthz-smoke-43762983-a/__tests__/route.test.ts` — Complete test suite for endpoint A
...
**In scope:**
- Create test file at `src/app/api/healthz-smoke-43762983-a/__tests__/route.test.ts`
...
- Modifications to the implementation (VRTX-0329)
...
**File:** `src/app/api/healthz-smoke-43762983-a/__tests__/route.test.ts`
...
- **RH-01:** Returns HTTP 200 status
  - Call GET() and verify `res.status === 200`
- **RH-02:** Response body matches spec `{ ok: true, variant: "43762983" }`
  - Call GET(), parse JSON, verify `json.ok === true` and `json.variant === "43762983"`
- **RH-03:** Content-Type header is application/json
  - Call GET() and verify `res.headers.get('Content-Type') === 'application/json'`
- **RH-04:** Response status is success (res.ok === true)
  - Call GET() and verify `res.ok === true`

#### Category 2: Type Safety (2 tests)
- **RH-05:** Field `ok` is boolean (not string, number, or null)
  - Parse JSON, verify `typeof json.ok === 'boolean'` and value is `true`
- **RH-06:** Field `variant` is string (not number or object)
  - Parse JSON, verify `typeof json.variant === 'string'` and value is exactly `"43762983"`

#### Category 3: HTTP Headers & Metadata (2 tests)
- **RH-07:** Response is NextResponse instance
  - Verify `res instanceof NextResponse`
- **RH-08:** Response has no authentication-related headers
  - Call GET(), verify `res.headers.get('WWW-Authenticate')` is null
  - Verify no `Set-Cookie` headers

#### Category 4: Performance (3 tests)
- **RH-09:** Single call response time < 100ms
  - Measure performance.now() before and after GET(), verify delta < 100
- **RH-10:** Sequential calls (3×) all < 100ms
  - Call GET() three times, measure each, all must be < 100ms
- **RH-11:** Under concurrent load (50 calls), all complete within 100ms
  - Create 50 parallel GET() calls, verify all complete and respond with 200

#### Category 5: Public Access & Consistency (3 tests)
- **RH-12:** No authentication required
  - Call GET() without auth headers/cookies, verify status 200
- **RH-13:** Multiple sequential calls return identical responses
  - Call GET() 3 times, parse JSON for each, verify all are identical: `{ ok: true, variant: "43762983" }`
- **RH-14:** Variant identifier is exactly "43762983"
  - Call GET(), parse JSON, verify `json.variant === "43762983"` (not "43762983-a", not other variants)
...
describe('GET /api/healthz-smoke-43762983-a', () => {
  beforeEach(() => {
...
- [ ] Test file `src/app/api/healthz-smoke-43762983-a/__tests__/route.test.ts` created
...
- [ ] All 14 tests pass locally: `npm run test src/app/api/healthz-smoke-43762983-a/__tests__/route.test.ts`
...
```bash
npm run test src/app/api/healthz-smoke-43762983-a/__tests__/route.test.ts
```
...
**Depends on:**
- VRTX-0329 — Implement healthz-smoke-43762983-a endpoint (must complete first)
