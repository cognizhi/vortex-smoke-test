# Bug Specification: Missing /healthz-smoke-bugfix2-357681766 Endpoint
...
The variant-specific health check endpoint `/healthz-smoke-bugfix2-357681766` is missing from the application. When accessed, it returns HTTP 404 instead of the expected HTTP 200 with a JSON response identifying the build variant.

This endpoint is needed for deployment verification and monitoring system integration to confirm that the bugfix2 variant (357681766) is deployed and reachable in production environments.
...
2. Make a GET request to `/healthz-smoke-bugfix2-357681766`
   - Command: `curl http://localhost:3000/healthz-smoke-bugfix2-357681766`
3. **Expected:** HTTP 200 response with JSON body: `{ "ok": true, "variant": "357681766" }`
...
The endpoint `/healthz-smoke-bugfix2-357681766` does not exist in the codebase. Checking the Next.js App Router structure:
- Expected location: `/workspace/repo/src/app/api/healthz-smoke-bugfix2-357681766/route.ts`
- Current status: **Directory does not exist**

The application has similar variant endpoints (e.g., `/healthz-smoke-963602537`, `/healthz-smoke-688707801`) implemented following the pattern from SPRINT-0005 onwards. The endpoint for variant "357681766" is missing from this established series.
...
Create a new self-contained route handler at `/src/app/api/healthz-smoke-bugfix2-357681766/route.ts` that:

1. **Implements a GET handler** that returns HTTP 200 with JSON response:
   - Body: `{ "ok": true, "variant": "357681766" }`
...
4. **Create comprehensive tests** at `/src/app/api/healthz-smoke-bugfix2-357681766/__tests__/route.test.ts`:
   - HTTP 200 status verification
   - Exact response body validation: `{ "ok": true, "variant": "357681766" }`
...
| Deployment verification | Low | New endpoint only affects bugfix2 variant monitoring |
...
- **FIX-01:** GET `/healthz-smoke-bugfix2-357681766` responds with HTTP 200 status code
- **FIX-02:** Response body is exactly `{ "ok": true, "variant": "357681766" }` with no extra fields
...
- **FIX-07:** Regression test `GET /healthz-smoke-bugfix2-357681766 returns 200` passes
...
2. **Field types** — `ok` is boolean `true`, `variant` is string `"357681766"`
...
- [ ] Create directory: `/src/app/api/healthz-smoke-bugfix2-357681766/`
- [ ] Implement: `/src/app/api/healthz-smoke-bugfix2-357681766/route.ts`
  - [ ] Export async GET handler
  - [ ] Return NextResponse.json with { ok: true, variant: "357681766" }
  - [ ] Set status code to 200
  - [ ] Include JSDoc header
- [ ] Create directory: `/src/app/api/healthz-smoke-bugfix2-357681766/__tests__/`
- [ ] Implement: `/src/app/api/healthz-smoke-bugfix2-357681766/__tests__/route.test.ts`
