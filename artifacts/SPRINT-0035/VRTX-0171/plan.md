# Plan: Missing /healthz-smoke-bugfix2-357681766 Endpoint
...
The variant-specific health check endpoint `/healthz-smoke-bugfix2-357681766` is missing from the application. When accessed, it returns HTTP 404 instead of the expected HTTP 200 with JSON response identifying the build variant. This blocks deployment verification and monitoring system integration.

---

## Root Cause

- **Missing file**: `/src/app/api/healthz-smoke-bugfix2-357681766/route.ts` does not exist
...
1. Responds to GET requests at `/healthz-smoke-bugfix2-357681766`
2. Returns HTTP 200 with JSON body `{ "ok": true, "variant": "357681766" }`
...
- **Implementation**:
  ...
  - Return `NextResponse.json({ ok: true, variant: "357681766" }, { status: 200 })`
...
- Manual test: `curl http://localhost:3000/healthz-smoke-bugfix2-357681766`
...
|------|---------|--------------|
| `src/app/api/healthz-smoke-bugfix2-357681766/route.ts` | Endpoint handler | NextResponse (built-in) |
| `src/app/api/healthz-smoke-bugfix2-357681766/__tests__/route.test.ts` | Test suite | Vitest, NextResponse |
...
- ✅ FIX-01: GET `/healthz-smoke-bugfix2-357681766` responds with HTTP 200
- ✅ FIX-02: Response body is exactly `{ "ok": true, "variant": "357681766" }`
...
| Monitoring | Low | Only affects bugfix2 variant monitoring |
