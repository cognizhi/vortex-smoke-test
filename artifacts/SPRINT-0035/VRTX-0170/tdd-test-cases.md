# TDD Test Cases: Missing /healthz-smoke-bugfix-494516155 Endpoint
...
| RH-02 | Route | Response Body | Returns correct JSON structure `{ "ok": true, "variant": "494516155" }` | route.test.ts | Red |
...
| RH-06 | Route | Type Safety | `variant` field is string "494516155" (not number) | route.test.ts | Red |
...
- **RH-02**: Verify exact response structure matches spec — `{ "ok": true, "variant": "494516155" }`
...
- **RH-06**: Verify `variant` is string "494516155" (not number 494516155)
...
1. Endpoint does not exist at `src/app/api/healthz-smoke-bugfix-494516155/route.ts`
...
1. Implement `src/app/api/healthz-smoke-bugfix-494516155/route.ts` with GET handler
