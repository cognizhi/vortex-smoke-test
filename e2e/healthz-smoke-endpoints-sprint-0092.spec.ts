...
test.describe('Healthz smoke endpoints — SPRINT-0092 (509572604)', () => {
  ...
  test('GET /api/healthz-smoke-509572604-b returns 200 with ok and variant', async ({
    request,
  }) => {
    const response = await request.get('/api/healthz-smoke-509572604-b')
    expect(response.status()).toBe(200)
    const body = await response.json()
    expect(body).toEqual({ ok: true, variant: '509572604' })
  })

  test('GET /api/healthz-smoke-509572604-c returns 200 with ok and variant', async ({
    request,
  }) => {
    const response = await request.get('/api/healthz-smoke-509572604-c')
    expect(response.status()).toBe(200)
    const body = await response.json()
    expect(body).toEqual({ ok: true, variant: '509572604' })
  })
