...
- `/api/healthz-smoke-cancel-892262901` — Returns `{ ok: true, variant: "892262901" }` (SPRINT-0025)
...
# SPRINT-0025: Variant smoke test endpoint (892262901)

**Sprint Goal:** Add a lightweight variant-specific smoke test endpoint for deployment verification.

## Feature: GET /api/healthz-smoke-cancel-892262901
...
This endpoint follows the established pattern from previous variant endpoints (SPRINT-0001 through SPRINT-0024) to maintain consistency and operational familiarity.

### Acceptance Criteria

✅ **Endpoint exists and responds**
- GET `/api/healthz-smoke-cancel-892262901` responds with HTTP 200
- Response body: `{ ok: true, variant: "892262901" }`
...
- Uses Next.js App Router convention: `src/app/api/healthz-smoke-cancel-892262901/route.ts`
- Variant identifier "892262901" is hardcoded in the response
...
- Single GET endpoint for variant "892262901"
...
- **Path:** `GET /api/healthz-smoke-cancel-892262901`
...
- Location: `src/app/api/healthz-smoke-cancel-892262901/route.ts`
- Export async `GET` handler function
- Use Next.js `NextResponse.json()` API
- Response structure: `{ ok: true, variant: "892262901" }`
- Include JSDoc header documenting the endpoint
- No guards, middleware, or conditional logic needed

**Test Coverage**
- Unit test file: `src/app/api/healthz-smoke-cancel-892262901/__tests__/route.test.ts`
- Tests should verify:
  - HTTP 200 status code
  - Exact response body: `{ ok: true, variant: "892262901" }`
...
|--------|------|-------|--------|------|-------|
| VRTX-0124 | EPIC | Add /healthz-smoke-cancel-892262901 endpoint | — | VST-0018 | Sprint container |
| VRTX-0125 | FEATURE | Implement /healthz-smoke-cancel-892262901 GET endpoint | VRTX-0124 | — | Feature specification |
| VRTX-0126 | TASK | Implement and test /healthz-smoke-cancel-892262901 endpoint | VRTX-0125 | — | Development work |
...
### 2026-07-05 — SPRINT-0025: Variant smoke test endpoint (892262901)

**Added:**
- New variant-specific health check endpoint `/api/healthz-smoke-cancel-892262901`
- Returns `{ ok: true, variant: "892262901" }` with zero dependencies
...
- Pattern follows established variant endpoint model from SPRINT-0001 through SPRINT-0024
