...
- `/api/healthz-smoke-cancel-498421836` — Returns `{ ok: true, variant: "498421836" }` (SPRINT-0020)
...
# SPRINT-0020: Variant smoke test endpoint (498421836)

**Sprint Goal:** Add a lightweight variant-specific smoke test endpoint for deployment verification.

## Feature: GET /api/healthz-smoke-cancel-498421836
...
This endpoint follows the established pattern from previous variant endpoints (SPRINT-0001 through SPRINT-0019) to maintain consistency and operational familiarity.

### Acceptance Criteria

✅ **Endpoint exists and responds**
- GET `/api/healthz-smoke-cancel-498421836` responds with HTTP 200
- Response body: `{ ok: true, variant: "498421836" }`
...
- Uses Next.js App Router convention: `src/app/api/healthz-smoke-cancel-498421836/route.ts`
- Variant identifier "498421836" is hardcoded in the response
...
- Single GET endpoint for variant "498421836"
...
- **Path:** `GET /api/healthz-smoke-cancel-498421836`
...
- Location: `src/app/api/healthz-smoke-cancel-498421836/route.ts`
- Export async `GET` handler function
- Use Next.js `NextResponse.json()` API
- Response structure: `{ ok: true, variant: "498421836" }`
- Include JSDoc header documenting the endpoint
- No guards, middleware, or conditional logic needed

**Test Coverage**
- Unit test file: `src/app/api/healthz-smoke-cancel-498421836/__tests__/route.test.ts`
- Tests should verify:
  - HTTP 200 status code
  - Exact response body: `{ ok: true, variant: "498421836" }`
...
|--------|------|-------|--------|------|-------|
| VRTX-0107 | EPIC | Add /healthz-smoke-cancel-498421836 endpoint | — | VST-0015 | Sprint container |
| VRTX-0108 | FEATURE | Implement /healthz-smoke-cancel-498421836 GET endpoint | VRTX-0107 | — | Feature specification |
| VRTX-0109 | TASK | Add GET /healthz-smoke-cancel-498421836 endpoint implementation | VRTX-0108 | — | Development work |
| VRTX-0110 | TASK | Add tests for /healthz-smoke-cancel-498421836 endpoint | VRTX-0108 | — | Testing work |
...
### 2026-07-05 — SPRINT-0020: Variant smoke test endpoint (498421836)

**Added:**
- New variant-specific health check endpoint `/api/healthz-smoke-cancel-498421836`
- Returns `{ ok: true, variant: "498421836" }` with zero dependencies
...
- Pattern follows established variant endpoint model from SPRINT-0001 through SPRINT-0019
